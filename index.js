...
const {
  ...
  */
  ...
  async function starts() {
	  ...
	  Lxa.on('group-participants-update', async (anu) => {
		  ...
		  try {
			  ...
			  if (anu.action == 'add') {
				  ...
				  teks = `hola gordito @${num.split('@')[0]}, portate bien o .kick`
	...
	error: {
	stick: 'no rompas las bolas, más tarde',
	Iv: 'Error en el link'
	},
	only: {
  ...
  */
  ...
  case 'namae':
	if (args.length < 1) return reply('pero escribi el nombre la concha de tu madre')
  Lxa.updatePresence(from, Presence.composing)
  if (!isRegister) return reply(mess.only.daftarB)
  ...
  //--fake reply
  case 'fitnah':
  if (args.length < 1) return reply(`Uso :\n${prefix}fitnah [@tag|mensaje de la persona q tageaste|lo que va a decir el bot]]\n\nEx : \n${prefix}fitnah @tagmember|puto|cerra el ojete mogolico`)
  var gh = body.slice(7)
  ...
  case 'warn':
	if (!isRegister) return reply(mess.only.daftarB)
  Lxa.sendMessage(from, `@${id.split('@')[0]} volves a hacer eso y te vas kickeado gordito`, text, {
	quoted: mek
  })
  break
  ...
  */
  ...
  if (!isBotGroupAdmins) return reply(mess.only.Badmin)
  Lxa.groupUpdateDescription(from, `${body.slice(9)}`)
  Lxa.sendMessage(from, 'cambio la descripción del grupo', text, {
	quoted: mek
  })
  break
...
reply(`        ────────────────\ne puto de mierda*\nese comando no esta en mi lista: *${prefix}${command}*\npone *${prefix}menu* para ver todos los comandos pq parece q te cuesta\n        ────────────────`)
				  }
